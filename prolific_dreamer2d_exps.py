...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument('--work_dir', type=str, default='work_dir/grad_exps', help='Working directory')
    ...
    ### ours 
    parser.add_argument('--plus_ratio', type=float, default=0.1, help='plus_ratio for ours')
    parser.add_argument('--plus_random', type=str2bool, default=True, help='plus_random for ours')
...
def main():
    ...
    for step, chosen_t in enumerate(pbar):
        ...
        for mode in ['SDS', 'ASD', 'CSD', 'VSD']:
            cross_attention_kwargs = {'scale': args.lora_scale} if (mode == 'VSD' and args.phi_model == 'lora') else {}
            if mode in ['SDS', 'VSD', 'CSD']:
                if mode == 'SDS':
                    guidance_scale = 100.
                    generation_mode = 'sds'
                elif mode == 'VSD':
                ...
                grad_, noise_pred, noise_pred_phi = sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings_vsd, t, \
                                                        guidance_scale=guidance_scale, unet_phi=unet_phi, \
                                                            generation_mode=generation_mode, phi_model=args.phi_model, \
                ...
                target = (latents_vsd - grad_).detach()
                loss_2 = 0.5 * F.mse_loss(latents_vsd, target, reduction="mean") / args.batch_size

            # record norm
            norm_dict[mode].append(grad_.norm().item())
    ...
    # save norm, in a pickle file
    import pickle
    with open(f'{args.work_dir}/norm_dict.pkl', 'wb') as f:
