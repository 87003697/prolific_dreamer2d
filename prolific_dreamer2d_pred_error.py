...
def get_parser(**parser_kwargs):
    ...
    ### ours 
    parser.add_argument('--plus_ratio', type=float, default=0.9, help='plus_ratio for ours')
    parser.add_argument('--plus_random', type=str2bool, default=False, help='plus_random for ours')
    ...
    if args.generation_mode != 'ours':
        ...
        args.work_dir = f'{args.work_dir}/{args.prompt}_plus_ratio_{args.plus_ratio}_random_{args.plus_random}_version_{args.version}_cfg_{args.guidance_scale}_cfg_qt_{args.guidance_scale_qt}_{args.run_id}'

    os.makedirs(args.work_dir, exist_ok=True)
    assert args.generation_mode in ['t2i', 'sds', 'vsd', 'ours', 'csd']
...
def main():
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            if args.generation_mode in ['sds', 'vsd', 'csd']:
                ...
                t_plus = t + torch.tensor([delta_t]).to(device)
                t_plus = torch.clamp(t_plus, min=args.t_start, max=args.t_end, ).type(torch.long)

                noisy_latents_qt = scheduler.add_noise(latents_vsd, noise, t_plus)
                grad_, noise_pred, noise_pred_phi = vsd_minus_grad_diffuser(unet, noisy_latents, noisy_latents_qt, noise, text_embeddings_vsd, t, t_plus, \
                                                                            guidance_scale=args.guidance_scale, guidance_scale_qt=args.guidance_scale_qt, \
                                                                            grad_scale=loss_weights[int(t)], grad_scale_qt=loss_weights[int(t_plus)], \
    ...
    # evaluate phi model in terms of the noise prediction ###
    # random sample one particle
    particle = particles[:1]
    # from 1 to num_train_timesteps, extract 100 timesteps with uniform interval
    noise = torch.randn_like(particle) # keep fixed noise
    timesteps = np.linspace(1, num_train_timesteps, 50, dtype=int)
    for t in timesteps:
        t = torch.tensor([t]).to(device)
        # call the noise prediction function
        with torch.no_grad():
            noisy_latents = scheduler.add_noise(particle, noise, t)
            text_embeddings_vsd = torch.cat([uncond_embeddings[:1], text_embeddings[:1]])
            grad_, noise_pred, noise_pred_phi = sds_vsd_grad_diffuser(unet, noisy_latents, noise, text_embeddings_vsd, t, \
                                                            guidance_scale=1., unet_phi=unet_phi, \
                                                                generation_mode=args.generation_mode, phi_model=args.phi_model, \
                                                                    cross_attention_kwargs=cross_attention_kwargs, \
                                                                        multisteps=args.multisteps, scheduler=scheduler, lora_v=args.lora_vprediction, \
                                                                            half_inference=args.half_inference, \
                                                                                cfg_phi=args.cfg_phi, grad_scale=args.grad_scale)
        error_pred = torch.norm(noise_pred - noise).detach()
        error_pred_phi = torch.norm(noise_pred_phi - noise).detach()

        norm_dict['pre-trained'].append(error_pred.item())
        norm_dict['fine-tuned'].append(error_pred_phi.item())

        print(f'pre-trained: {error_pred.item()}, fine-tuned: {error_pred_phi.item()}')

    import pickle
    with open(f'{args.work_dir}/error_dict.pkl', 'wb') as f:
